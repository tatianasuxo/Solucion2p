...
__device__ const uint8_t sBox[8][4][16] = {
    {
...
__device__ uint8_t DES::Substitution(uint8_t sBoxNo, uint8_t inputVal)
{
...
__device__ chunk64 DES::InitPerm(chunk64 asd)
{
    chunk64 result;
    result.val = 0;
    uint8_t index = 1;
    for (uint8_t i = 0; i < 8; i++)
    {
        for (uint8_t j = 0; j < 8; j++)
        {
            ...
            uint8_t bit = GETBIT(asd.arr[7 - j], index);
...
__device__ chunk64 DES::FinalPerm(chunk64 asd)
{
    chunk64 result;
    result.val = 0;
    uint8_t size = 8;
...
__device__ chunk64 DES::Expansion(chunk32 we)
{
    chunk64 result;
    result.val = 0;
...
__device__ chunk32 DES::SBoxPermutation(chunk32 in)
{
    chunk32 result;
    result.val = 0;
...
__device__ chunk64 DES::PermutedChoice1(chunk64 key)
{
    chunk64 result;
    result.val = 0;
...
__device__ chunk64 DES::PermutedChoice2(chunk64 key)
{
    chunk64 result;
    result.val = 0;
...
__device__ uint32_t DES::RotateHalfkeyLeft(chunk32 halfKey, uint8_t n)
{
...
__device__ uint8_t rotNumber(uint8_t round) // It's short, so can be converted into constarray for speed
{
    switch(round)
    {
        case 0:
...
__device__ chunk32 DES::Feistel(chunk32 data, chunk64 subKey)
{
...
__device__ chunk64 DES::Process(chunk64 key, chunk64 data, bool isDecode)
{
...
__device__ chunk64 DES::Encode(chunk64 key, chunk64 data)
{
    return Process(key, data, false);
}

__device__ chunk64 DES::Decode(chunk64 key, chunk64 data)
{
